...
describe("Timeout and Cancellation Integration Tests", () => {
    ...
    describe("Error Handler Integration", () => {
        ...
        it("should handle TimeoutError in error handler middleware", async () => {
            ...
            framework.addPipelineDefinition({
                ...
                middlewareStack: [
                    ...
                    {
                        activationContext: {
                            activate: async (contextBinnacle: any, response: any) => {
                                if (response.errorStack && response.errorStack.length > 0) {
                                    errorHandlerCalled = true;
                                    capturedError = response.errorStack[0];

                                    if (capturedError instanceof TimeoutError) {
                                        return {
                                            error: "Request Timeout",
                                            statusCode: 408,
                                            timeout: capturedError.timeout,
                                            elapsed: capturedError.elapsed
                                        };
                                    }
                                }
                                return null;
        ...
        it("should handle CancellationError in error handler middleware", async () => {
            ...
            framework.addPipelineDefinition({
                ...
                middlewareStack: [
                    ...
                    // Business logic that won't execute
                    {
                        activationContext: {
                            activate: async () => {
                                return { success: true };
                        ...
                        activationContext: {
                            activate: async (contextBinnacle: any, response: any) => {
                                if (response.errorStack && response.errorStack.length > 0) {
                                    ...
                                    if (capturedError instanceof CancellationError) {
                                        return {
                                            error: "Client Closed Request",
                                            statusCode: 499,
                                            reason: capturedError.reason
                                        };
                                    }
                                }
                                return null;
            ...
            // Error handler should have been called
            expect(errorHandlerCalled).toBe(true);
            expect(capturedError).toBeInstanceOf(CancellationError);
            expect((capturedError as CancellationError).reason).toBe("User cancelled");
        });
    });
